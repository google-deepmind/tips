...
for CHECKPOINT in "${TIPS_CHECKPOINTS[@]}"; do
  ...
  wget https://storage.googleapis.com/tips_data/v1_0/checkpoints/scenic/${CHECKPOINT}_vision.npz
  echo "Downloading ${CHECKPOINT} (text encoder weights)"
  wget https://storage.googleapis.com/tips_data/v1_0/checkpoints/scenic/${CHECKPOINT}_text.npz
done

for CHECKPOINT in "${TIPS_V2_CHECKPOINTS[@]}"; do
  echo "Downloading ${CHECKPOINT} (vision encoder weights)"
  wget https://storage.googleapis.com/tips_data/v2_0/checkpoints/scenic/${CHECKPOINT}_vision.npz
  echo "Downloading ${CHECKPOINT} (text encoder weights)"
  wget https://storage.googleapis.com/tips_data/v2_0/checkpoints/scenic/${CHECKPOINT}_text.npz
done
